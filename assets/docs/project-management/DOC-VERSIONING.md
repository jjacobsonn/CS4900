...
|--------|----------|---------|
| **Sprint 0 baseline** | `sprint-0/` | Unchanged snapshot of architecture, requirements, API, DB from the original baseline sprint. |
| **Revised / new specs** | `sprint-1/` (or `revision/`) | Updated architecture, versioning, admin, API changes. |
...
Optional: one-time copy of `sprint-0/` to `archive/sprint-1-baseline/` if you want a frozen backup.
...
- Git already keeps history; a single "baseline" folder (`sprint-0/`) is enough to mean "before revisions."

So: **old = sprint-0 (unchanged). New = sprint-1 (or revision/).** No version suffixes in doc names unless you really want one file per major version (e.g. `architecture-v2.md`) and are okay updating references.
